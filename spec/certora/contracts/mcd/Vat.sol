...
contract Vat {
    ...
    function rely(address usr) public note auth { require(live == 1, "Vat/not-live"); wards[usr] = 1; }
    function deny(address usr) public note auth { require(live == 1, "Vat/not-live"); wards[usr] = 0; }
    modifier auth {
    ...
    function hope(address usr) public note { can[msg.sender][usr] = 1; }
    function nope(address usr) public note { can[msg.sender][usr] = 0; }
    function wish(address bit, address usr) internal virtual view returns (bool) {
    ...
    modifier note {
        _;
        assembly {
            // log an 'anonymous' event with a constant 6 words of memory
